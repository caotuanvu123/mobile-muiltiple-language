...
class StatisticalController extends Controller
{
    public function index(StatisticalRequest $request)
    {
        ...
        if ($request->input('type') == $request::TYPE_CHOOSE_DAY) {
            $bills = Order::select(DB::raw('DATE(created_at) AS date'), DB::raw('sum(price) AS total'))
...
//                Month
                ...
                if (!count($bills) > 0) {
                    ...
                    return view('admin.statistical.index', ['data' => json_encode($data), 'request' => $request->all()]);
                }
        ...
        }elseif($request->input('type') == $request::TYPE_MONTH) {
            for ($i = 1; $i <= 12; $i++) {
                $dataArr[$i] = [
                    'total' => 0
                ];
            }
            $bills = Order::select(DB::raw('MONTH(created_at) AS date'), DB::raw('sum(price) AS total'))
                ->where('type', '=', Order::PAID)
        ...
        } elseif($request->input('type') == $request::TYPE_DAY) {
//            Type day
...
//            Type year
            ...
            $bills = Order::select(DB::raw('YEAR(created_at) AS date'), DB::raw('sum(price) AS total'))
                ->where('type', '=', Order::PAID)
                ->groupBy('date')
                ->get();
